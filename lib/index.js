// dsh-plugin-template — nodo host (Cordis).
//
...
//      through `settingsScope` (pattern of dsh-credit-meter / dsh-sound-notify).
...
function apply(ctx, config) {
  ...
  ctx.on("ready", () => {
    console.log(
      `[dsh-plugin-template] attivo (enabled=${state.enabled}, order=${resolved.order})`
    );
  });
  ctx.on("dispose", () => {
    console.log("[dsh-plugin-template] rimosso");
  });
  ctx.effect(
